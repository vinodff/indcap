                  ...
                  <div className="absolute bottom-3 left-3 z-40 flex items-center gap-1 backdrop-blur-md bg-black/70 border border-white/10 rounded-xl px-2 py-1.5 shadow-xl">
                    ...
                    <div className="relative flex items-center">
                      <button
                        ...
                        className={`flex items-center gap-1 px-2 py-1 rounded-l-lg text-[10px] font-bold uppercase tracking-wider transition-all ${autoCameraEnabled ? 'bg-cyan-600/80 text-white' : 'bg-white/5 text-white/30 hover:bg-white/10 hover:text-white/60'}`}
                      >
                      ...
                      <button
                        ...
                        className={`px-1 py-1 rounded-r-lg border-l border-black/20 transition-all ${cameraSettingsOpen ? 'bg-cyan-500 text-white' : autoCameraEnabled ? 'bg-cyan-600/80 text-white/80 hover:text-white' : 'bg-white/5 text-white/30 hover:bg-white/10 hover:text-white/60'}`}
                      >
                        <ChevronDown size={11} className={`transition-transform ${cameraSettingsOpen ? 'rotate-180' : ''}`} />
                      </button>
                      {cameraSettingsOpen && (
                        <div className="absolute bottom-full left-0 mb-2 w-64 p-3 rounded-xl bg-zinc-900/95 backdrop-blur-md border border-white/10 shadow-2xl text-white space-y-3" onClick={e => e.stopPropagation()}>
                          ...
                          {/* Style preset */}
                          <div>
                            <div className="text-[10px] uppercase tracking-wider text-white/40 mb-1">Style</div>
                            <div className="grid grid-cols-3 gap-1">
                              {([
                                ...
                                <button key={val}
                                  ...
                                  className={`px-1.5 py-1 rounded-md text-[10px] font-semibold transition-all ${cameraStyle === val ? 'bg-cyan-500 text-white' : 'bg-white/5 text-white/50 hover:bg-white/10'}`}
                                >{label}</button>
                              ))}
                            </div>
                          ...
                          <label className="block">
                            <div className="flex justify-between text-[10px] text-white/50 mb-0.5"><span>Intensity</span><span className="text-white/70">{Math.round(cameraIntensity * 100)}%</span></div>
                          ...
                          {/* Shake */}
                          <label className="block">
                            <div className="flex justify-between text-[10px] text-white/50 mb-0.5"><span>Shake</span><span className="text-white/70">{Math.round(cameraShake * 100)}%</span></div>
                          ...
                          {/* Max zoom */}
                          <label className="block">
                            <div className="flex justify-between text-[10px] text-white/50 mb-0.5"><span>Max Zoom</span><span className="text-white/70">{cameraMaxZoom.toFixed(2)}×</span></div>
                          ...
                          <button onClick={() => setCameraTrackFaces(v => !v)}
                            ...
                            {cameraTrackFaces ? <ToggleRight size={18} className="text-cyan-400" /> : <ToggleLeft size={18} className="text-white/30" />}
                          </button>
                        </div>
                      )}
                    </div>
