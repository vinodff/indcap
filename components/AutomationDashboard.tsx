...
const AutomationDashboard: React.FC<Props> = ({ onClose }) => {
  ...
  const getStatusColor = (status: LogStatus) => {
      switch (status) {
          case 'RECEIVED': return 'text-[var(--cc-blue)]';
          case 'PROCESSING': return 'text-yellow-400';
          case 'SENT': return 'text-green-400';
          case 'SKIPPED': return 'text-[var(--cc-text-3)]';
          case 'ERROR': return 'text-red-500';
          default: return 'text-[var(--cc-text-3)]';
      }
  };
  ...
  const getHealthBadge = (status: HealthStatus) => {
      switch (status) {
          ...
          case 'PAUSED': return <div className="flex items-center gap-1 text-[var(--cc-text-3)] font-bold bg-gray-500/20 px-2 py-0.5 rounded text-[10px] border border-gray-500/30"><Pause size={12}/> PAUSED</div>;
      }
  };
  ...
  return (
    <div className="fixed inset-0 z-[90] bg-[#050505] flex flex-col animate-in slide-in-from-bottom-10 duration-300 font-sans text-gray-200">
      ...
      {/* Header */}
      <div className="h-16 border-b border-[var(--cc-border)] flex items-center justify-between px-6 bg-[var(--cc-bg)]">
          <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                  ...
                  <div>
                      <h2 className="text-sm font-bold text-[var(--cc-text-1)] uppercase tracking-wider">Automation Hub</h2>
                      <div className="flex items-center gap-2">
                          <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>
                          <span className="text-[10px] text-[var(--cc-text-3)]">System Online</span>
                      </div>
                  </div>
              </div>
              
              {/* Tabs */}
              <div className="flex bg-[#161616] p-1 rounded-lg border border-[var(--cc-border)]">
                  <button onClick={() => setActiveTab('LIVE')} className={`px-4 py-1.5 rounded text-xs font-bold transition-all ${activeTab === 'LIVE' ? 'bg-[var(--cc-surface-3)] text-[var(--cc-text-1)] shadow-sm' : 'text-[var(--cc-text-3)] hover:text-[var(--cc-text-2)]'}`}>Live Console</button>
                  <button onClick={() => setActiveTab('ANALYTICS')} className={`px-4 py-1.5 rounded text-xs font-bold transition-all ${activeTab === 'ANALYTICS' ? 'bg-[var(--cc-surface-3)] text-[var(--cc-text-1)] shadow-sm' : 'text-[var(--cc-text-3)] hover:text-[var(--cc-text-2)]'}`}>Analytics Report</button>
              </div>
          </div>
          
          <div className="flex items-center gap-4">
              {getHealthBadge(stats.healthStatus)}
              <button onClick={onClose} className="p-2 hover:bg-[var(--cc-surface-3)] rounded-full text-[var(--cc-text-3)] hover:text-[var(--cc-text-1)] transition-colors">
                  <X size={20} />
              </button>
          </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#050505] p-6">
              ...
              {activeTab === 'LIVE' && (
                  <div className="flex flex-col h-full gap-6">
                      ...
                      {/* Console Logs */}
                      <div className="flex-1 bg-[var(--cc-bg)] rounded-xl border border-[var(--cc-border)] overflow-hidden flex flex-col">
                          <div className="p-3 border-b border-[var(--cc-border)] bg-[#101010] flex justify-between items-center">
                              <div className="flex items-center gap-2 text-xs font-bold text-[var(--cc-text-3)]">
                          ...
                          <div className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-xs">
                              ...
                              {logs.map(log => (
                                  <div key={log.id} className="flex gap-3 p-2 hover:bg-white/5 rounded border border-transparent hover:border-[var(--cc-border)]/50 transition-colors group">
                                      ...
                                      <div className="flex-1 space-y-1">
                                          <div className="flex items-center gap-2">
                                              ...
                                              <span className="text-[var(--cc-text-3)]">[{log.type}]</span>
                                              <span className="text-[var(--cc-text-2)]">User: <span className="text-[var(--cc-text-1)] font-bold">{log.user}</span></span>
                                          </div>
                                          <div className="text-[var(--cc-text-3)] pl-4 border-l border-[var(--cc-border)]">
                                              {log.content && <div className="text-[var(--cc-text-2)] italic">"{log.content}"</div>}
                                              <div>{log.details}</div>
                                              {log.actionTaken && (
              ...
              {activeTab === 'ANALYTICS' && (
                  <div className="space-y-6 max-w-5xl mx-auto">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                          ...
                          <div className="lg:col-span-2 bg-[var(--cc-bg)] border border-[var(--cc-border)] rounded-xl p-6">
                              <div className="flex justify-between items-center mb-6">
                                  <div>
                                      <h3 className="text-sm font-bold text-[var(--cc-text-1)]">Performance Trend</h3>
                                      <p className="text-xs text-[var(--cc-text-3)]">DMs sent vs Replies (Last 7 Days)</p>
                                  </div>
                                  <div className="flex gap-2">
                                      <div className="flex items-center gap-1 text-[10px] text-[var(--cc-text-3)]"><div className="w-2 h-2 rounded-full bg-blue-500"></div> DMs</div>
                                      <div className="flex items-center gap-1 text-[10px] text-[var(--cc-text-3)]"><div className="w-2 h-2 rounded-full bg-green-500"></div> Replies</div>
                                  </div>
                              </div>
                          ...
                          {/* Funnel */}
                          <div className="bg-[var(--cc-bg)] border border-[var(--cc-border)] rounded-xl p-6">
                              <h3 className="text-sm font-bold text-[var(--cc-text-1)] mb-6">Conversion Funnel</h3>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          ...
                          {/* Trigger Performance */}
                          <div className="bg-[var(--cc-bg)] border border-[var(--cc-border)] rounded-xl p-6">
                              <h3 className="text-sm font-bold text-[var(--cc-text-1)] mb-1">Trigger Effectiveness</h3>
                              <p className="text-xs text-[var(--cc-text-3)] mb-6">Which strategy gets more replies?</p>
                              
                              <div className="space-y-4">
                                  {stats.analytics.triggerPerformance.map((t, idx) => (
                                      <div key={idx} className="flex items-center gap-4">
                                          <div className="w-24 text-xs font-bold text-[var(--cc-text-3)] uppercase">{t.type.replace('_', ' ')}</div>
                                          <div className="flex-1">
                                              <div className="h-2 bg-[var(--cc-surface-3)] rounded-full overflow-hidden">
                                                  <div className="h-full bg-blue-500 rounded-full" style={{width: `${t.conversionRate}%`}}></div>
                                              </div>
                                          </div>
                                          <div className="w-12 text-xs font-bold text-[var(--cc-text-1)] text-right">{t.conversionRate}%</div>
                                      </div>
                                  ))}
                              </div>
                          </div>
          ...
          <div className="w-[300px] bg-[var(--cc-bg)] border-l border-[var(--cc-border)] flex flex-col p-6 shadow-xl z-10">
              
              {/* Health Panel */}
              <div className="mb-8">
                  <h3 className="text-xs font-black text-[var(--cc-text-3)] uppercase tracking-widest mb-4 flex items-center gap-2">
                      <ShieldCheck size={12} /> Account Health
                  </h3>
                  <div className="bg-[#101010] border border-[var(--cc-border)] rounded-xl p-4 space-y-4">
                       <div className="flex justify-between items-center">
                           <span className="text-xs text-[var(--cc-text-3)]">Hourly Limit</span>
                           <span className="text-xs font-mono font-bold text-[var(--cc-text-1)]">{stats.hourlyUsage} / 30</span>
                       </div>
                       <div className="w-full h-1 bg-[var(--cc-surface-3)] rounded-full overflow-hidden">
                       ...
                       <div className="flex justify-between items-center pt-2 border-t border-[var(--cc-border)]">
                           <span className="text-xs text-[var(--cc-text-3)]">Daily Limit</span>
                           <span className="text-xs font-mono font-bold text-[var(--cc-text-1)]">{stats.dailyUsage} / 100</span>
                       </div>
                       <div className="w-full h-1 bg-[var(--cc-surface-3)] rounded-full overflow-hidden">
              ...
              <div className="flex-1">
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                          <label className="text-[10px] font-bold text-[var(--cc-text-3)] uppercase">Simulate As</label>
                          <div className="flex items-center bg-black border border-[var(--cc-border)] rounded-lg px-3 py-2">
                              <User size={14} className="text-[var(--cc-text-3)] mr-2" />
                              <input 
                                type="text" 
                                value={simUser} 
                                onChange={e => setSimUser(e.target.value)}
                                className="bg-transparent text-xs text-[var(--cc-text-1)] outline-none w-full placeholder-gray-700"
                      ...
                      {/* Trigger: Comment */}
                      <div className="p-3 bg-[var(--cc-surface)]/50 rounded-xl border border-[var(--cc-border)] space-y-3">
                          <div className="flex items-center gap-2 text-xs font-bold text-[var(--cc-text-1)]">
                              <MessageCircle size={14} className="text-blue-500" /> New Comment
                          </div>
                          <textarea 
                            value={testComment}
                            onChange={e => setTestComment(e.target.value)}
                            className="w-full h-16 bg-black border border-[var(--cc-border)] rounded-lg p-2 text-xs text-[var(--cc-text-1)] outline-none focus:border-blue-500 resize-none"
                            placeholder="Type comment..."
                          />
                          <button 
                            onClick={() => handleSimulate('COMMENT')}
                            disabled={!testComment}
                            className="w-full py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed text-[var(--cc-text-1)] text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-2"
                          >
                             Trigger Event <ChevronRight size={12} />
                          </button>
                          <div className="flex gap-2 justify-center">
                              <button onClick={() => setTestComment("Link pls!")} className="text-[9px] px-2 py-1 bg-[var(--cc-surface-3)] rounded text-[var(--cc-text-3)] hover:text-[var(--cc-text-1)]">"Link"</button>
                              <button onClick={() => setTestComment("Cheap likes here")} className="text-[9px] px-2 py-1 bg-[var(--cc-surface-3)] rounded text-[var(--cc-text-3)] hover:text-[var(--cc-text-1)]">Spam</button>
                          </div>
                      </div>

                      {/* Other Triggers */}
                      <div className="grid grid-cols-2 gap-2">
                          <button onClick={() => handleSimulate('STORY')} className="p-3 bg-[var(--cc-surface)]/50 hover:bg-[var(--cc-surface-3)] border border-[var(--cc-border)] rounded-xl flex flex-col items-center justify-center gap-1 transition-all">
                              <Zap size={16} className="text-yellow-500" />
                              <span className="text-[9px] font-bold text-[var(--cc-text-2)]">Story Reply</span>
                          </button>
                          <button onClick={() => handleSimulate('FOLLOW')} className="p-3 bg-[var(--cc-surface)]/50 hover:bg-[var(--cc-surface-3)] border border-[var(--cc-border)] rounded-xl flex flex-col items-center justify-center gap-1 transition-all">
                              <Bot size={16} className="text-purple-500" />
                              <span className="text-[9px] font-bold text-[var(--cc-text-2)]">Follow</span>
                          </button>
                      </div>
                  </div>
              </div>
              ...
              {/* Footer Controls */}
              <div className="pt-6 border-t border-[var(--cc-border)] mt-auto">
                 <button 
...
const MetricCard = ({ label, value, icon }: { label: string, value: number, icon: React.ReactNode }) => (
    <div className="bg-[var(--cc-bg)] border border-[var(--cc-border)] p-4 rounded-xl flex flex-col items-center justify-center">
        <div className="flex items-center gap-2 mb-1">
            {icon}
            <span className="text-2xl font-black text-[var(--cc-text-1)]">{value}</span>
        </div>
        <span className="text-[10px] font-bold text-[var(--cc-text-3)] uppercase tracking-wide">{label}</span>
    </div>
);
...
const AnalyticsCard = ({ title, value, subtext, trend, trendUp, icon }: { title: string, value: string | number, subtext?: string, trend?: string, trendUp?: boolean, icon: React.ReactNode }) => (
    <div className="bg-[var(--cc-bg)] border border-[var(--cc-border)] p-5 rounded-xl">
        <div className="flex justify-between items-start mb-2">
            <div className="p-2 bg-[var(--cc-surface)] rounded-lg">{icon}</div>
            {trend && (
        ...
        <h3 className="text-2xl font-black text-[var(--cc-text-1)]">{value}</h3>
        <p className="text-xs font-bold text-[var(--cc-text-3)] uppercase mt-1">{title}</p>
...
const FunnelStep = ({ label, value, color, width }: { label: string, value: number, color: string, width: string }) => (
    <div className="w-full">
        <div className="flex justify-between text-xs mb-1 px-1">
            <span className="text-[var(--cc-text-3)] font-medium">{label}</span>
            <span className="text-[var(--cc-text-1)] font-bold">{value}</span>
        </div>
        <div className="h-8 bg-[var(--cc-surface)] rounded-lg overflow-hidden mx-auto" style={{width: width}}>
...
const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-[#1a1a1a] border border-[var(--cc-border)] p-3 rounded-lg shadow-xl">
                <p className="text-xs font-bold text-[var(--cc-text-1)] mb-2">{label}</p>
                <p className="text-xs text-[var(--cc-blue)]">DMs: {payload[0].value}</p>
