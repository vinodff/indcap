...
const SocialPublisher: React.FC<Props> = ({ onClose, videoSrc, captions }) => {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/95 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in zoom-in-95 duration-200">
      <div className="bg-[var(--cc-surface)] border border-[var(--cc-border)] rounded-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden shadow-2xl relative">
        
        <button onClick={onClose} className="absolute top-4 right-4 p-2 bg-[var(--cc-surface-3)] rounded-full text-[var(--cc-text-3)] hover:text-[var(--cc-text-1)] z-20">
            <X size={20} />
        </button>

        {/* HEADER */}
        <div className="p-8 border-b border-[var(--cc-border)] bg-[#161616]">
            <h2 className="text-2xl font-black text-[var(--cc-text-1)] flex items-center gap-3">
                <UploadCloud className="text-blue-500" /> Multi-Platform Publisher
            </h2>
            <p className="text-[var(--cc-text-3)] text-sm mt-1">Upload once. Go viral everywhere.</p>
            
            {/* Steps */}
            <div className="flex gap-2 mt-6">
                <button 
                    onClick={() => !isUploading && setActiveStep('ACCOUNTS')}
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeStep === 'ACCOUNTS' ? 'bg-blue-600 text-[var(--cc-text-1)]' : 'text-[var(--cc-text-3)] hover:bg-[var(--cc-surface-3)]'}`}
                >
                    1. Connect Accounts
                </button>
                <div className="w-8 h-[2px] bg-[var(--cc-surface-3)] self-center"></div>
                <button 
                    onClick={() => connectedCount > 0 && setActiveStep('UPLOAD')}
                    disabled={connectedCount === 0}
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeStep === 'UPLOAD' ? 'bg-blue-600 text-[var(--cc-text-1)]' : 'text-gray-600'}`}
                >
                    2. Upload & Publish
                </button>
            </div>
        ...
        {/* CONTENT */}
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8 bg-[var(--cc-bg)]">
            
            {activeStep === 'ACCOUNTS' && (
                <div className="max-w-2xl mx-auto space-y-4">
                    <div className="bg-blue-900/20 border border-blue-500/30 p-4 rounded-xl flex items-start gap-3 mb-6">
                        <Lock size={18} className="text-[var(--cc-blue)] mt-0.5" />
                        <div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {accounts.map(acc => {
                            ...
                            return (
                                <div key={acc.id} className="bg-[#161616] border border-[var(--cc-border)] p-4 rounded-xl flex items-center justify-between group hover:border-[var(--cc-border)] transition-colors">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            {acc.status === 'CONNECTED' ? (
                                                ...
                                                <p className="text-xs text-[var(--cc-text-3)]">{conf.rules}</p>
                                            )}
                                        </div>
                                    </div>
                                    <button 
                                        ...
                                        className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${
                                            acc.status === 'CONNECTED' ? 'bg-[var(--cc-surface-3)] text-[var(--cc-text-3)] hover:text-red-400' : 
                                            acc.status === 'CONNECTING' ? 'bg-[var(--cc-surface-3)] text-[var(--cc-text-3)]' :
                    ...
                    {!isUploading ? (
                        <>
                            ...
                            <div className="flex-1 space-y-8 min-w-[300px]">
                                <div>
                                    <h3 className="text-sm font-bold text-[var(--cc-text-3)] uppercase tracking-widest mb-4">Select Destinations</h3>
                                             ...
                                             return (
                                                 <button 
                                                    ...
                                                    className={`w-full p-3 rounded-xl border flex items-center gap-3 transition-all ${
                                                        isSelected ? 'bg-blue-600/10 border-blue-500 text-[var(--cc-text-1)]' : 'bg-[#161616] border-[var(--cc-border)] text-[var(--cc-text-3)] hover:bg-[var(--cc-surface-3)]'
                                                    }`}
                                                 >
                                                     <div className={`w-5 h-5 rounded border flex items-center justify-center ${isSelected ? 'bg-blue-500 border-blue-500' : 'border-gray-600'}`}>
                                                         {isSelected && <CheckCircle2 size={14} className="text-[var(--cc-text-1)]" />}
                                                     </div>
                                ...
                                <div>
                                    <h3 className="text-sm font-bold text-[var(--cc-text-3)] uppercase tracking-widest mb-4">Metadata Check</h3>
                                    <div className="bg-[#161616] border border-[var(--cc-border)] rounded-xl p-4 space-y-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-16 h-24 bg-[var(--cc-surface-3)] rounded-lg overflow-hidden">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs font-bold text-[var(--cc-text-3)] flex items-center gap-1"><AlertCircle size={12}/> SEO Auto-Gen</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className={`flex-[2] ${!isInstagramSelected ? 'opacity-50 pointer-events-none grayscale' : ''} transition-all duration-300`}>
                                 <div className="flex items-center justify-between mb-4">
                                     <h3 className="text-sm font-bold text-[var(--cc-text-3)] uppercase tracking-widest flex items-center gap-2">
                                         <Zap size={16} className={isInstagramSelected ? "text-pink-500" : "text-gray-600"} /> 
                                         Instagram Automation
                                     </h3>
                                     {isInstagramSelected && (
                                         <div className="flex items-center gap-2">
                                            <span className="text-xs text-[var(--cc-text-3)]">{igAutomation.enabled ? 'Automation ON' : 'Automation OFF'}</span>
                                            <button 
                                 ...
                                 {isInstagramSelected && (
                                    <div className="mb-4 bg-[#161616] border border-[var(--cc-border)] rounded-lg p-3 flex justify-between items-center">
                                       <div className="flex items-center gap-2 text-xs text-[var(--cc-text-2)]">
                                          <AlertCircle size={14} className="text-[var(--cc-blue)]" />
                                          <span>Account Classification:</span>
                                       </div>
                                       <select 
                                         value={igAutomation.accountType}
                                         onChange={(e) => updateAccountType(e.target.value as any)}
                                         className="bg-black border border-[var(--cc-border)] rounded text-xs text-[var(--cc-text-1)] px-2 py-1 outline-none"
                                       >
                                 ...
                                 <div className={`grid grid-cols-1 lg:grid-cols-2 gap-6 transition-all ${igAutomation.enabled ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                                     ...
                                     <div className="space-y-4">
                                         ...
                                         {/* Comment Automation */}
                                         <div className="bg-[#161616] border border-[var(--cc-border)] rounded-xl overflow-hidden">
                                             <div className="p-4 border-b border-[var(--cc-border)] flex items-center justify-between bg-[var(--cc-surface)]/50">
                                                 <div className="flex items-center gap-2">
                                                     <MessageCircle size={16} className="text-[var(--cc-blue)]" />
                                             ...
                                             {igAutomation.comment.enabled && (
                                                 <div className="p-4 space-y-4">
                                                     <div className="space-y-2">
                                                         <label className="text-[10px] text-[var(--cc-text-3)] uppercase font-bold">Trigger Logic</label>
                                                         <div className="grid grid-cols-3 gap-2">
                                                             {['ANY', 'KEYWORDS', 'AI_INTENT'].map(type => (
                                                                 <button 
                                                                    key={type}
                                                                    onClick={() => setIgAutomation(prev => ({...prev, comment: {...prev.comment, triggerType: type as any}}))}
                                                                    className={`py-1.5 px-2 rounded text-[10px] font-bold border transition-colors ${igAutomation.comment.triggerType === type ? 'bg-blue-600/20 border-blue-500 text-[var(--cc-blue)]' : 'border-[var(--cc-border)] text-[var(--cc-text-3)] hover:bg-[var(--cc-surface-3)]'}`}
                                                                 >
                                                     ...
                                                     {igAutomation.comment.triggerType === 'KEYWORDS' && (
                                                         <div>
                                                             <input 
                                                                 ...
                                                                 className="w-full bg-black border border-[var(--cc-border)] rounded-lg p-2 text-xs text-[var(--cc-text-1)] focus:border-blue-500 outline-none"
                                                             />
                                                         </div>
                                                     )}
                                                     ...
                                                     <div className="space-y-2">
                                                         <div className="flex justify-between items-center">
                                                            <label className="text-[10px] text-[var(--cc-text-3)] uppercase font-bold">DM Response</label>
                                                            <div className="flex gap-2">
                                                                <button onClick={() => setIgAutomation(prev => ({...prev, comment: {...prev.comment, dmMode: 'MANUAL'}}))} className={`text-[9px] px-2 py-0.5 rounded ${igAutomation.comment.dmMode === 'MANUAL' ? 'bg-gray-700 text-[var(--cc-text-1)]' : 'text-[var(--cc-text-3)]'}`}>Manual</button>
                                                                <button onClick={() => handleGenerateDm('comment')} className={`text-[9px] px-2 py-0.5 rounded flex items-center gap-1 ${igAutomation.comment.dmMode === 'AI_SMART' ? 'bg-purple-600 text-[var(--cc-text-1)]' : 'text-[var(--cc-text-3)]'}`}>
                                                         ...
                                                         <textarea 
                                                            ...
                                                            className="w-full h-16 bg-black border border-[var(--cc-border)] rounded-lg p-2 text-xs text-[var(--cc-text-2)] focus:border-blue-500 outline-none resize-none"
                                                            placeholder="DM Text..."
                                                         />
                                                     </div>
                                                     ...
                                                     <div className="flex items-center justify-between">
                                                         <div className="flex items-center gap-1">
                                                             <Clock size={12} className="text-[var(--cc-text-3)]" />
                                                             <span className="text-[10px] text-[var(--cc-text-3)]">Delay</span>
                                                         </div>
                                                         <select 
                                                            value={igAutomation.comment.delay}
                                                            onChange={(e) => setIgAutomation(prev => ({...prev, comment: {...prev.comment, delay: e.target.value as any}}))}
                                                            className="bg-black border border-[var(--cc-border)] rounded text-[10px] px-2 py-1 text-[var(--cc-text-1)] outline-none"
                                                         >
                                         ...
                                         <div className="grid grid-cols-2 gap-4">
                                             <div className="bg-[#161616] border border-[var(--cc-border)] rounded-xl p-3">
                                                 <div className="flex justify-between items-center mb-2">
                                                     <div className="flex items-center gap-1.5">
                                                         <Zap size={14} className="text-yellow-400" />
                                                         <span className="text-xs font-bold text-[var(--cc-text-2)]">Story Reply</span>
                                                     </div>
                                                     <button 
                                                 ...
                                                 {igAutomation.story.enabled && (
                                                     <>
                                                         <div className="flex justify-end mb-2">
                                                             <button onClick={() => handleGenerateDm('story')} className={`text-[9px] px-2 py-0.5 rounded flex items-center gap-1 ${igAutomation.story.dmMode === 'AI_SMART' ? 'bg-purple-600 text-[var(--cc-text-1)]' : 'bg-gray-700 text-[var(--cc-text-3)]'}`}>
                                                         ...
                                                         <textarea 
                                                            ...
                                                            className="w-full h-12 bg-black border border-[var(--cc-border)] rounded-lg p-2 text-[10px] text-[var(--cc-text-2)] focus:border-yellow-500 outline-none resize-none"
                                             ...
                                             <div className="bg-[#161616] border border-[var(--cc-border)] rounded-xl p-3">
                                                 <div className="flex justify-between items-center mb-2">
                                                     <div className="flex items-center gap-1.5">
                                                         ...
                                                         <span className="text-xs font-bold text-[var(--cc-text-2)]">Welcome DM</span>
                                                     </div>
                                                     <button 
                                                  ...
                                                  {igAutomation.welcome.enabled && (
                                                      <>
                                                         <div className="flex justify-end mb-2">
                                                             <button onClick={() => handleGenerateDm('welcome')} className={`text-[9px] px-2 py-0.5 rounded flex items-center gap-1 ${igAutomation.welcome.dmMode === 'AI_SMART' ? 'bg-purple-600 text-[var(--cc-text-1)]' : 'bg-gray-700 text-[var(--cc-text-3)]'}`}>
                                                         ...
                                                         <textarea 
                                                            ...
                                                            className="w-full h-12 bg-black border border-[var(--cc-border)] rounded-lg p-2 text-[10px] text-[var(--cc-text-2)] focus:border-purple-500 outline-none resize-none"
                                         ...
                                         <div className="bg-[#161616] border border-[var(--cc-border)] rounded-xl overflow-hidden">
                                              <button 
                                                  ...
                                                  className="w-full flex items-center justify-between p-3 bg-[var(--cc-surface)]/30 hover:bg-[var(--cc-surface)]/50 transition-colors"
                                              >
                                                  <div className="flex items-center gap-2">
                                                      <ShieldCheck size={14} className="text-green-400" />
                                                      <span className="text-xs font-bold text-[var(--cc-text-2)]">Advanced Safety</span>
                                                  </div>
                                                  {isSafetyCollapsed ? <ChevronDown size={14} className="text-[var(--cc-text-3)]" /> : <ChevronUp size={14} className="text-[var(--cc-text-3)]" />}
                                              </button>
                                              
                                              {!isSafetyCollapsed && (
                                                  <div className="p-3 space-y-3 bg-black/20">
                                                      <div className="flex items-center justify-between">
                                                          <span className="text-[10px] text-[var(--cc-text-3)]">Max DMs/Hour</span>
                                                          <div className="flex items-center gap-2">
                                                            <span className="text-[10px] text-[var(--cc-text-3)] font-mono">{igAutomation.safety.maxDmsPerHour}</span>
                                                            <div className="w-16 h-1 bg-[var(--cc-surface-3)] rounded-full overflow-hidden">
                                                                <div className="h-full bg-blue-500" style={{width: `${(igAutomation.safety.maxDmsPerHour/50)*100}%`}}></div>
                                                            </div>
                                                          </div>
                                                      </div>
                                                       <div className="flex items-center justify-between">
                                                          <span className="text-[10px] text-[var(--cc-text-3)]">Max DMs/Day</span>
                                                          <div className="flex items-center gap-2">
                                                            <span className="text-[10px] text-[var(--cc-text-3)] font-mono">{igAutomation.safety.maxDmsPerDay}</span>
                                                            <div className="w-16 h-1 bg-[var(--cc-surface-3)] rounded-full overflow-hidden">
                                                      ...
                                                      <div className="flex items-center justify-between">
                                                          <span className="text-[10px] text-[var(--cc-text-3)]">Avoid Duplicates</span>
                                     ...
                                     <div className="relative">
                                          <div className="sticky top-0">
                                              <h3 className="text-xs font-bold text-[var(--cc-text-3)] uppercase tracking-widest mb-3 flex items-center gap-2">
                                                  <Bell size={12} /> Live Preview
                                              </h3>
                                              
                                              {/* Phone Mockup */}
                                              <div className="bg-white text-black rounded-3xl border-8 border-[var(--cc-border)] overflow-hidden shadow-2xl max-w-[280px] mx-auto">
                                                  {/* Status Bar */}
                                                  ...
                                                  <div className="bg-gray-50 pb-4">
                                                      ...
                                                      <div className="aspect-[4/5] bg-gray-200 relative mb-2">
                                                          ...
                                                          <div className="absolute bottom-2 left-2 flex gap-2">
                                                              ...
                                                              <div className="text-[8px] text-[var(--cc-text-1)] font-bold drop-shadow-md pt-1">@creator</div>
                                                          </div>
                                                      </div>
                                                          ...
                                                          {igAutomation.enabled && igAutomation.comment.enabled && (
                                                              <div className="animate-in slide-in-from-top-2 fade-in duration-700 delay-500">
                                                                  <div className="flex justify-center mb-2">
                                                                      <span className="text-[8px] text-[var(--cc-text-3)] bg-gray-200 px-2 py-0.5 rounded-full">
                                                                  ...
                                                                  <div className="flex gap-2 flex-row-reverse">
                                                                      ...
                                                                      <div className="flex-1 bg-blue-500 text-[var(--cc-text-1)] p-2 rounded-lg rounded-tr-none shadow-md">
                                              ...
                                              <p className="text-center text-[10px] text-[var(--cc-text-3)] mt-3">Preview of customer experience</p>
                                          </div>
                                     </div>
                        ...
                        <div className="w-full max-w-2xl mx-auto space-y-6">
                             <div className="flex items-center justify-between mb-4">
                                 <h3 className="text-xl font-bold text-[var(--cc-text-1)]">Publishing Progress</h3>
                             ...
                             <div className="space-y-4">
                                 {selectedPlatforms.map(pid => {
                                     ...
                                     return (
                                         <div key={pid} className="bg-[#161616] border border-[var(--cc-border)] p-4 rounded-xl">
                                             <div className="flex justify-between items-center mb-2">
                                                 <div className="flex items-center gap-3">
                                                     <div className="p-1.5 rounded bg-[var(--cc-surface-3)]" style={{color: conf.color}}>{conf.icon}</div>
                                                     <span className="font-bold text-sm">{conf.label}</span>
                                                 </div>
                                                 <span className={`text-xs font-bold ${
                                                     prog.stage === 'DONE' ? 'text-green-500' : 
                                                     prog.stage === 'FAILED' ? 'text-red-500' : 'text-[var(--cc-blue)]'
                                                 }`}>
                                                     {prog.stage === 'DONE' ? 'PUBLISHED' : prog.stage}
                                                 </span>
                                             </div>
                                             
                                             <div className="h-2 bg-[var(--cc-surface-3)] rounded-full overflow-hidden mb-2">
                                                 <div 
                                             ...
                                             <div className="flex justify-between items-center">
                                                 <span className="text-[10px] text-[var(--cc-text-3)]">
                                                 ...
                                                 {prog.stage === 'DONE' && (
                                                     <a href={prog.url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[10px] text-[var(--cc-blue)] hover:underline">
                                             ...
                                             {isIg && igAutomation.enabled && prog.stage === 'DONE' && (
                                                 <div className="mt-3 pt-3 border-t border-[var(--cc-border)] flex flex-col gap-2 animate-in fade-in slide-in-from-top-2">
                                                     <div className="flex items-center gap-2">
                                                         <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                                                         <span className="text-[10px] font-bold text-[var(--cc-text-3)]">Automation Active:</span>
                                                         <span className="text-[10px] text-[var(--cc-text-3)]">Listening for {igAutomation.comment.triggerType === 'KEYWORDS' ? 'keywords' : 'comments'}...</span>
                                                     </div>
                                                     <button 
                                                        onClick={() => setShowLiveDashboard(true)}
                                                        className="w-full py-1.5 bg-blue-900/30 hover:bg-blue-900/50 border border-blue-500/30 text-[var(--cc-blue)] text-xs font-bold rounded-lg transition-colors flex items-center justify-center gap-2"
                                                     >
                             ...
                             {allDone && (
                                 <div className="mt-8 p-6 bg-green-900/20 border border-green-500/30 rounded-xl text-center animate-in zoom-in duration-300">
                                     <div className="w-12 h-12 bg-green-500 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg shadow-green-500/20">
                                         <CheckCircle2 size={24} className="text-[var(--cc-text-1)]" />
                                     </div>
                                     <h3 className="text-xl font-bold text-[var(--cc-text-1)]">You're Live! 🚀</h3>
                                     <p className="text-sm text-[var(--cc-text-3)] mt-2">Your video has been successfully published to all selected platforms.</p>
                                     <button onClick={onClose} className="mt-6 px-6 py-2 bg-[var(--cc-surface-3)] hover:bg-[var(--cc-surface-3)]/80 rounded-lg text-[var(--cc-text-1)] font-bold text-sm transition-colors">
                                         Done
                                     </button>
                                 </div>
                             )}
        ...
        {/* FOOTER */}
        {activeStep === 'ACCOUNTS' && (
             <div className="p-4 border-t border-[var(--cc-border)] bg-[#161616] flex justify-end">
                 <button 
                    onClick={() => connectedCount > 0 && setActiveStep('UPLOAD')}
                    disabled={connectedCount === 0}
